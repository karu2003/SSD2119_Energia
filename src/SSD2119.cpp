 ...
 *  In this mode only 263K colors (RGB666) are supported, no matter what
 * ENTRY_MODE is set
 *
 *  GFX used the 16 bit RGB565 format which gets converted to 24 bit RGB888
 * format then truncated to RGB666
 *
...
SSD2119::SSD2119(int DC, int CS, int RST)
    : Adafruit_GFX(SSD2119_LCD_HORIZONTAL_MAX, SSD2119_LCD_VERTICAL_MAX) {
  _p_rs = DC;
  _p_cs = CS;
  _p_rst = RST;
  _palette = RGB666;
...
void SSD2119::SSD2119Write(unsigned char ucDC, unsigned long ulInstruction,
                           unsigned char ucRegType) {

#if defined(__AVR__) || defined(__LM4F120H5QR__) ||                            \
...
void SSD2119::ClearDisplay(unsigned long ulBackgroundColor) {
  for (unsigned long ulCount = 0; ulCount < ((SSD2119_LCD_HORIZONTAL_MAX) *
                                             long(SSD2119_LCD_VERTICAL_MAX));
       ulCount++) {
...
void SSD2119::SPI_init() {
  ...
  digitalWrite(_p_cs, HIGH);
    ...
    defined(PART_TM4C129XNCZAD) || defined(PART_TM4C1294NCPDT)
  digitalWrite(_p_cs, HIGH);
  SPI.begin();
  SPI.setClockDivider(SPI_CLOCK_DIV8); // MAX 15 MHz SPI Clock
  ...
  SPI.begin();
#endif
#if defined(_LIB_SAM_) // Due extended SPI has chip select pin as a parameter in
                       // all calls
  // Enable SPI streaming
  SPI.begin(_p_cs);
#endif
...
void SSD2119::DrawOnePixel(unsigned short usX, unsigned short usY,
...
const uint16_t scans[] = {
    SSD2119_OUTPUT_ORIGIN_DOWNRIGHT, SSD2119_OUTPUT_ORIGIN_UPLEFT,
...
void SSD2119::fillRect(int16_t x, int16_t y, int16_t w, int16_t h,
                       uint16_t color) {
  for (int16_t i = 0; i < h; i++)
    drawFastHLine(x, y + i, w, color);
}

void SSD2119::drawRect(int16_t x, int16_t y, int16_t w, int16_t h,
                       uint16_t color) {
