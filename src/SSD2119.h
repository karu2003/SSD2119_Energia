/*
  ...
  REV 4
    ...
    4 - Added support for Arduino Due and its extended SPI functions
  ...
  Developer:         TheFax
  This class contains:
  SSD2119_init()                                      --> Setup of Color LCD
  display based on SSD2119 DrawOnePixel(X,Y,Color)                      --> Draw
  only one pixel This class uses the 4 wire SPI bus of the LCD display.
  Connections:
  ...
  for extended SPI on Die CS MUST be 4,10 or 52 SDI        |    on SPI connector
  | (SPI transfer, from uP to LCD) // SDA on shield

  Date:              Marsh 2021
  Hardware target:   TM4C123G -> 4-wire SPI bus -> SSD2119
  Framework          Energia
  Developer:         Andrew Buckin
*/
...
private:
  ...
  void SSD2119Write(unsigned char ucDC, unsigned long ulInstruction,
                    unsigned char ucRegType);
  void SPI_init();
  void DrawOnePixel(unsigned short usX, unsigned short usY,
